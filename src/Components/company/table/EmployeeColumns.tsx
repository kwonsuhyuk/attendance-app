...
export const getEmployeeColumns = (): ColumnDef<EmployeeInfo>[] => {
  return employeeColumnsConfig.map(column => ({
    ...
    cell: ({ row }) => {
      ...
      return (
        <div
          className={` ${column.key === "email" ? "whitespace-normal break-all" : ""} ${column.key === "salaryAmount" ? "max-x-[80px] whitespace-normal break-words" : ""} ${["name"].includes(column.key) ? "block truncate" : ""} `}
